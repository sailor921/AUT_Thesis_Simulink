...
time_samples = 120; % number of samples in time period
...
A = [zeros(N - 1, 1), eye(N - 1); 1, zeros(1, N - 1)];
%% simulation

time = 0; % initialize time value
time_step = 0.025; % define time step
...
while iteration <= time_samples
    ...
    % controller value
    [U_i, W_i] = Controllers(P_g(:, :, iteration), P_star_g, Theta_g(:, :, iteration), Theta_g_hat(:, :, iteration), kp, A);

    % derivatives variables

    [Theta_g_hat_dot] = Observers(Theta_g_hat(:, :, iteration), Theta_g(:, :, iteration), kt, A);
...
imwrite(A, map, 'b.gif', 'gif', 'DelayTime', time_step, 'LoopCount', inf); %save file output
...
for i = 2:time_samples
    ...
    im = frame2im(fr);
    [A, map] = rgb2ind(im, 256);
    imwrite(A, map, 'b.gif', 'gif', 'WriteMode', 'append', 'DelayTime', time_step); %save file output
    pause(time_step)
end
