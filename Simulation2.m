...
imwrite(AA, map, 'b2.gif', 'gif', 'DelayTime', time_step, 'LoopCount', inf); %save file output
...
for i = 2:time_samples
    ...
    im = frame2im(fr);
    [AA, map] = rgb2ind(im, 256);
    imwrite(AA, map, 'b2.gif', 'gif', 'WriteMode', 'append', 'DelayTime', time_step); %save file output
    pause(time_step)
end
