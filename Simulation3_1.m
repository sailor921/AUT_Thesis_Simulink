...
Theta_g(:, :, 1) = (rand(1, N) - 0.5)*pi; % initialize agent orientation randomly
...
for i = 1:N
    ...
    anArrow.Parent = f.CurrentAxes;
    anArrow.Color = 'r';
    anArrow.Position = [P_g(1, i, 1), P_g(2, i, 1), 0.5 * cos(Theta_g(1, i, 1)), 0.5 * sin(Theta_g(1, i, 1))];
    Agents_Arrow  = [Agents_Arrow  anArrow];
...
imwrite(AA, map, 'b4.gif', 'gif', 'DelayTime', time_step, 'LoopCount', inf); %save file output
...
for i = 2:time_samples
    ...
    for j = 1:N
        ...
        Agents_Arrow(j).Position = [P_g(1, j, i), P_g(2, j, i), 0.5 * cos(Theta_g(1, j, i)), 0.5 * sin(Theta_g(1, j, i))];
    ...
    im = frame2im(fr);
    [AA, map] = rgb2ind(im, 256);
    imwrite(AA, map, 'b4.gif', 'gif', 'WriteMode', 'append', 'DelayTime', time_step); %save file output
    pause(time_step)
end
